...
# /lean:plan — Commit to an approach

You are writing the `## Plan` section of a feature spec. This is **convergent thinking** — pick one approach and lay out the concrete work. If the user still has open questions about direction, send them back to `/lean:discuss`.

## Inputs

- Optional feature argument (e.g. `/lean:plan 003`). Resolve via "Current feature discovery" (see `discuss.md`) if omitted.
- Optional free-text modifier from the invocation (e.g. `/lean:plan use Postgres instead`). Incorporate into the new plan.

## Reads (section-scoped)

1. **`.specs/config.yaml`** (falls back to `~/.claude/lean-spec-config.yaml` if not present; per-project wins when both exist) — `specs_root`, `project_hints`, `subagent_threshold`.
...
- **Testable** — `/lean:verify` must be able to tell whether it was done correctly.

Rule of thumb: 3–8 tasks for a typical feature. Fewer than 3 means the feature is too small to need a plan; go straight to `/lean:execute`. More than 8 usually means the feature should be decomposed into sibling docs via `part_of:`.

**Decomposition offer.** After drafting the plan, if the task count exceeds 8, use AskUserQuestion:
1. **"Suggest a decomposition into sibling features"** — you propose logical groupings, describe what each sibling spec would cover, and offer to create them via `/lean:new` with `part_of:` set. The user then works each sibling through the full workflow separately.
2. **"Keep as a single plan"** — proceed with writing the monolithic plan as drafted.

If the user chooses decomposition, outline the proposed sibling specs and stop — do not write the current Plan section. The user will run `/lean:new` for each sibling, then plan them individually.
...
- Suggested next step: `/lean:execute` (or `/lean:discuss` if planning surfaced fresh ambiguities).
...
- **Explore sub-agent for unfamiliar code.** Follow the `subagent_threshold` rule. Burning main-context tokens on bulk source reads is exactly the failure mode LeanSpec exists to avoid.
- **Never execute.** Planning does not edit code. If you catch yourself about to write a code change, stop — that's `/lean:execute`'s job.

...
> TBD — run `{{CMD_PREFIX}}spek:plan` to generate investigation tasks from the hypotheses above.

### Fix

> TBD — populated by `{{CMD_PREFIX}}spek:plan` after root cause is confirmed.

### Details

> TBD — populated by `{{CMD_PREFIX}}spek:plan`.
...
Next step: run {{CMD_PREFIX}}spek:plan to generate Investigation and Fix tasks
from the hypotheses above. Then run {{CMD_PREFIX}}spek:execute to work each task.
```
...
- **Canonical references.** Use `spek:<skill>` in internal guidance; `{{CMD_PREFIX}}spek:<skill>` in user-facing output and AskUserQuestion text. Never hardcode `/` or `$`.

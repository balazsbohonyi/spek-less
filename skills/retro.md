...
- Otherwise stop and tell the user to finish verification first via `{{CMD_PREFIX}}spek:verify`.
...
- Suggested next step: commit via `{{CMD_PREFIX}}spek:commit`, or start the next feature via `{{CMD_PREFIX}}spek:new`.

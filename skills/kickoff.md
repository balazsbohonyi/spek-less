...
- Set `part_of: <name-value>` in each scaffolded spec's frontmatter — read the `name:` field from `project.md` (a plain string like `SpekLess`); use the string value, NOT a file path.
...
- Suggested next step: `{{CMD_PREFIX}}spek:discuss <feature-number>` on the highest-priority feature, or manual review of `project.md` if they want to edit before proceeding

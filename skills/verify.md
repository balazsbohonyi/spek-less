...
- **If status is ISSUES_TO_FIX or INCOMPLETE:** use the **AskUserQuestion tool** with three options:
  1. **"Run `{{CMD_PREFIX}}spek:execute` now to fix the flagged issues"** (recommended)
  2. **"Revise the Plan first via `{{CMD_PREFIX}}spek:plan`"** (when the issues are structural)

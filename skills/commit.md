...
You are drafting a git commit message that summarizes the work done on a feature since the last commit, and (on user confirmation) creating the commit. You are the only skill besides `/lean:execute` that can write to the working tree, and the only skill anywhere that runs `git commit`.

This is a **convenience skill**, not a workflow step. The user invokes it whenever they want — after one task, after all tasks, after a verify-fix pass, never. LeanSpec does not force commits and never calls this skill automatically.

## Inputs

- Optional feature argument (e.g. `/lean:commit 003`). Resolve via current-feature discovery if omitted.
- Optional free-text modifier (e.g. `/lean:commit just the verify fixes`, `/lean:commit scope=task 3`). Use it to narrow what the draft summarizes.

## Reads (section-scoped)

1. **`.specs/config.yaml`** (falls back to `~/.claude/lean-spec-config.yaml` if not present; per-project wins when both exist) — `specs_root` and `commit_style`. `commit_style` is one of: `plain` (default), `conventional`, or a free-text custom rule the installer captured. Empty / missing = treat as `plain`.
...
- **Verify-fix entries** since the anchor (log entries produced after a `/lean:verify` pass flagged issues).
...
Keep it to one line of heading plus no body. The heading alone is enough for future `/lean:commit` runs to find the anchor.
...
Next: <suggestion — /lean:execute to continue, /lean:verify if all tasks are done, or stop>.
```
...
3. Suggest the user fix the underlying issue, then re-run `/lean:commit`.
4. Do NOT append a `Committed` entry to `execution.md` — the commit did not happen.

## Addressing verify-flagged commits

When `/lean:commit` runs after a verify→fix cycle, the execution log tail will contain entries from `/lean:execute` addressing the Verification section's issues. The draft should name those as fixes, not as a re-do of the original tasks. Example:
...
The cue is in the log entries: look for headings like `Fix: <something>` or `Addressing verify issue: <something>` produced by the prior `/lean:execute` run.
...
- **Never spawns sub-agents.** Same reasoning as `/lean:execute`: the user is watching a commit get drafted, and that oversight is the point.

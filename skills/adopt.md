---
name: lean:adopt
description: Retroactively create a spec for code that already exists — reverse-engineers Context and Plan from the actual files. Use when you built something without LeanSpec and want documentation + a verification baseline after the fact. May spawn an Explore sub-agent.
---

# /lean:adopt — Reverse-engineer a spec from existing code

You are creating a feature spec for code that was written WITHOUT LeanSpec and needs to be retroactively documented. The spec should read as if the author had used LeanSpec from the start: Context explains why the code exists, Plan describes the work as if it were planned up-front (with all tasks pre-checked since the work is done).

This is a capability neither GSD nor SpecKit offers. It's the main reason LeanSpec is useful on day one of installation in a mature repo.

## Inputs

- **Required argument:** a short description of what to adopt. Examples:
  - `/lean:adopt "the auth flow in src/auth/"`
  - `/lean:adopt "everything changed in the last 3 commits"`
  - `/lean:adopt "the billing module — files listed below" file1.ts file2.ts`
...
1. **Derive feature number and slug** the same way `/lean:new` does.
...
3. **Create `spec.md`** from the template, but populate it from what you learned:
   - **Frontmatter:**
     ...
     - `starting_sha:` set to the current git HEAD (short SHA). The diff baseline is "now," which means `/lean:verify` on an adopted feature becomes a documentation check rather than a diff check.
   ...
   - **`## Verification`:** leave empty. The user will run `/lean:verify` next.
4. **Do not create `execution.md`.** Adopted features have no execution history — the work predates LeanSpec. Leave it absent.
5. **If `project.md` exists**, add the same `> Part of [**{{project_name}}**](../project.md).` link under `## Context` that `/lean:new` does.
...
Next step: run /lean:verify to check that the Plan's task breakdown matches
...
- **Sub-agent for breadth.** When the scope is "a folder" or "a feature area," delegate to Explore. Burning main-context tokens on bulk source reads defeats the purpose of LeanSpec.

...
Next step: run {{CMD_PREFIX}}spek:verify to check that the Plan matches the code.
```
...
(`~~like this~~`) features you don't want. Then re-run {{CMD_PREFIX}}spek:adopt
...
Next step: run {{CMD_PREFIX}}spek:verify on individual specs to confirm Plans match code.
```

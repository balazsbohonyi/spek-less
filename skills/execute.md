...
- Optional feature argument (e.g. `/lean:execute 003`). Resolve via current-feature discovery if omitted.
- Optional free-text modifier (e.g. `/lean:execute address the issues verify flagged` or `/lean:execute skip task 3`). Interpret these as guidance, not as replacements for the Plan.

## Reads (section-scoped)

1. **`.specs/config.yaml`** (falls back to `~/.claude/lean-spec-config.yaml` if not present; per-project wins when both exist) — `specs_root`, `suggest_commits`, `project_hints`.
...
1. Capture the current git HEAD: `git rev-parse HEAD`. Write the short SHA to frontmatter `starting_sha:`. This is the audit anchor — `/lean:verify` later reads `git diff <starting_sha>..HEAD`.
...
2. Do NOT edit the Plan section yourself — that's `/lean:plan`'s job.
3. Stop and tell the user: "Plan needs revision. Run `/lean:plan <instructions>` to update the approach, then re-run `/lean:execute` to continue. I've recorded the correction in the log."
...
If invoked after `/lean:verify` flagged issues (the Verification section contains unresolved items):
...
5. Tell the user to re-run `/lean:verify` when done.

Do NOT rewrite the Verification section — that's `/lean:verify`'s job. Your job is to do the work; verify's job is to check it.
...
2. **Append a pause-point entry to `execution.md`.** Make it unambiguous:
   ...
   all changes reflected in this log and on disk. Resume with /lean:execute.
   ```
3. **Tell the user** what you did and that the session should be reset:
   > "I've completed tasks 1–3 and logged a pause point. Context is getting tight. Start a fresh session (or `/clear`) and run `/lean:execute` again — I'll read the log and pick up from task 4."
...
- Suggested next step: `/lean:verify` if all tasks done, `/lean:plan` if direction needs revision, or "re-run `/lean:execute` to continue" if you stopped for a contextual reason.
...
- **Never spawn sub-agents.** Execution happens in the main conversation, always. The reasons are about user experience, not technical capability (sub-agents *can* edit code — that's not the argument):
  1. **User intervention collapses inside a sub-agent.** LeanSpec's core UX is "the user can interrupt and steer at any point." If execution runs inside a sub-agent, the user only sees the final summary — they lose the ability to jump in mid-task and say "stop, use approach X instead."

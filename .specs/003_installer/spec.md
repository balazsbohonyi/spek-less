...
**Success criteria:**
- `ls .specs/` shows `config.yaml`, `principles.md`, and `templates/` after install
...
- **Templates dir is always overwritten on re-install** — gets latest framework templates; user edits belong in `.specs/`, not `templates/`
...
#### 3. Config generation
**Files:** `install.sh`, `templates/config.yaml.tmpl`
...
**Files:** `install.sh`
**Approach:** `cp -r templates/ "$SPECS_ROOT/templates/"` — always overwrite so re-installs get latest templates.
...
- Task 5 — Templates installation: ✓ — `install.sh:282-287` copies each `*.tmpl` file to `$SPECS_ROOT/templates/` on every run (always overwrites); directory is created with `mkdir -p`
...
**Goal check:** The installer achieves its stated goal. A developer can run `install.sh`, press Enter at every prompt, and get a correct working install in under 2 minutes. All four success criteria are satisfied: `.specs/` contains `config.yaml`, `principles.md`, and `templates/`; `.claude/commands/spek/` contains all skill files; no `{{PLACEHOLDER}}` strings remain (all 6 substituted); re-running with existing config preserves all values. One undocumented behavior: the installer also writes a `## SpekLess` block to `CLAUDE.md` (`install.sh:386-403`) — this is useful but not listed in the Plan. Not a bug, but the spec is incomplete.

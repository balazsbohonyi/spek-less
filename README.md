...
| **`/spek:recall`** | Searches prior spec decisions by natural-language query — grep-first, section-scoped reads of Context, Discussion, and Assumptions across all specs. Returns a brief synthesized answer followed by cited matches. Strictly read-only. |
...
## Walkthrough 4 — Bulk adopting a mature codebase

You've just installed SpekLess in an existing project with many undocumented features. Instead of running adopt one-at-a-time:

```
/spek:adopt
```

No argument triggers bulk discovery. Phase 1 scans the codebase using convention heuristics (route groups, page components, domain modules, store slices) and an Explore sub-agent, then writes `.specs/FEATURES.md`:

```markdown
# Discovered Features

> Generated by spek:adopt. Edit freely: reorder, reword, add features, or
> strike through (`~~like this~~`) features you don't want.

## 001: User Authentication
**Signals:** routes (`/login`, `/logout`), directory (`src/auth/`)
**Files:** `src/auth/session.ts` `src/middleware/auth.ts` `src/pages/Login.tsx`
**Summary:** Cookie-based session auth with route guarding.

## 002: Billing and Payments
**Signals:** routes (`/billing`, `/checkout`), directory (`src/billing/`)
**Files:** `src/billing/*` `src/pages/Billing.tsx`
**Summary:** Subscription management, Stripe integration.
```

You review and edit the file — reorder, reword, strike through features you don't want. Then:

```
/spek:adopt
```

Phase 2 reads the edited file, creates specs for each remaining feature (sequentially, with a checkpoint every 10), and offers to clean up FEATURES.md when done.
...
│   ├── FEATURES.md                    # bulk-adopt discovery list — intermediate, optional
│   ├── 001_auth-rewrite/
│   │   ├── spec.md
│   │   └── execution.md
...
Two worked examples and one sample artifact are included:
...
- **`examples/003_bulk-adopt/`** — a sample `FEATURES.md` showing what bulk discovery produces. Includes 4 realistic features (one struck-through) with Signals, Files, and Summary fields.
